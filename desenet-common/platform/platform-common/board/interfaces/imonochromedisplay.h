...
public:
	 ...
	 */
	virtual void clear( PixelColor background = White ) { UNUSED(background); }
	...
	virtual void writeDisplayMemory( size_t startAddress , const uint8_t * const sourceBitmap ,
									 size_t numberOfBits )
	{ UNUSED(startAddress); UNUSED(sourceBitmap); UNUSED(numberOfBits); }
	...
	{ UNUSED(startAddress); UNUSED(destinationBitmap); UNUSED(numberOfBits);}
	...
	{ UNUSED(begin); UNUSED(end); UNUSED(color); }
	...
	{ UNUSED(topLeft); UNUSED(size); UNUSED(color); }
	...
	{ UNUSED(topLeft); UNUSED(text); UNUSED(color); }
};
