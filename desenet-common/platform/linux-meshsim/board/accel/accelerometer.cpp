...
void Accelerometer::getAccelerationValues(int16_t & x, int16_t & y, int16_t & z)
{
	...
	//
	QRect desktopRect = QApplication::desktop()->availableGeometry();		// Desktop rectangle
	...
	x = slopeX * fabs(pointOfInterest.x()) - 2048;		// Calculate x
	y = slopeY * fabs(pointOfInterest.y()) - 2048;		// Calculate y
